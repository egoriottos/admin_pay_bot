...
public class CheckSubscriptionsAndKickUser {
    ...
    private void processExpiredForUser(User user, List<Subscription> subscriptions) {
        ...
        if (!expiredChannelNames.isEmpty()) {
            try {
                ...
                log.warn(
                        "Не удалось уведомить telegramId={} об истечении подписок (возможно, заблокировал бота): {}",
                        user.getTelegramId(),
                        e.getMessage());
            }
    ...
    private void sendReminderForUser(User user, List<Subscription> subscriptions) {
        try {
            ...
            log.warn("Не удалось отправить напоминание telegramId={} (возможно, заблокировал бота): {}",
                    user.getTelegramId(),
                    e.getMessage());
        }
